...
2. LOGICAL TIERS & ARCHITECTURAL HIERARCHY (CRITICAL):
   You must group resources into logical functional tiers within the main `AWS Cloud` wrapper. Blend this logical structure with network boundaries:
   - `Ingestion Tier` (e.g., API Gateway, Load Balancers, CloudFront).
   - `Agent Execution VPC` (The actual VPCs, containing Subnets, NAT/IGW, and compute resources like ECS Fargate or EC2).
   - `Model Inference Tier` (e.g., Amazon Bedrock, SageMaker).
   - `Graph State & Memory` (e.g., ElastiCache/Redis, RDS, DynamoDB).
   ...
   - *Subnet Classification:* Nest compute/data resources strictly inside their explicit subnets (Public/Private based on routing rules).
