...
// "local"). This machine has one persisted "owner" user (data/owner.json); the web
// UI operates as that owner, and every token it generates — plus the env token —
// maps to the owner userId, so all of the owner's tokens see the same sessions.
// The layout already carries userId everywhere, so multiple users (a different
// owner id) can be added later without changing call sites.
