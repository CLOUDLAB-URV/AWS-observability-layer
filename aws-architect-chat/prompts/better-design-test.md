...
1. GLOBAL LAYOUT, COMPACT RENDERING & EXTERNAL ACTORS:
   - Start the D2 code with the ELK layout engine configuration to optimize 2D space:
     `vars: { d2-config: { layout-engine: elk } }`
   - Use `direction: right` for the global flow.
...
4. DYNAMIC NAMING, VISUAL STYLING & GRID COMPACTING (SYNTAX CRITICAL):
   ...
   - Structural Compacting (CRITICAL): To prevent the diagram from growing infinitely to the right, group massive blocks (like the VPC and Event Bus) into invisible parallel containers (`wrapper_tier: "" { style.opacity: 0, grid-rows: 2 }`). Internally, enforce `grid-columns: X` or `grid-rows: X` within Tiers and AZs to stack resources compactly instead of lining them up horizontally.
...
5. LOGICAL TRAFFIC ROUTING & DATA FLOWS:
   - Flat Connection Declarations: Define ALL connections at the very bottom of the D2 script (outside the AWS and Client wrappers) using clean dot-notation aliases. Do NOT nest connections inside the group blocks.
     - *Example:* `device -> aws.infrastructure_tier.iot.core: "MQTT Telemetry"`
