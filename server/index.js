...
app.post('/api/chats/:chatId/deployments', agentGate, requireToken, async (req, res) => {
    ...
    try {
        ...
        const { svg, svgProtocol, svgActionSteps, svgProtocolSteps, svgNoLabels, hasSteps, error } = await renderDeployedDiagram(d2);
        broadcastToChat(req.userId, chatId, { type: 'render-svg', svg, svgProtocol, svgActionSteps, svgProtocolSteps, svgNoLabels, hasSteps, renderError: error });
...
app.get('/api/chats/:chatId/diagram', agentGate, requireSession, async (req, res) => {
    ...
    const { svg, svgProtocol, svgActionSteps, svgProtocolSteps, svgNoLabels, hasSteps, error } = await renderDeployedDiagram(d2);
    res.json({ chat: chatId, svg, svgProtocol, svgActionSteps, svgProtocolSteps, svgNoLabels, hasSteps, renderError: error });
});
...
vizWss.on('connection', (socket) => {
    socket.on('message', async (data) => {
        ...
        if (message.type === 'subscribe') {
            ...
            const { svg, svgProtocol, svgActionSteps, svgProtocolSteps, svgNoLabels, hasSteps, error } = await renderDeployedDiagram(d2);
            socket.send(JSON.stringify({ type: 'init', chat: chatId, svg, svgProtocol, svgActionSteps, svgProtocolSteps, svgNoLabels, hasSteps, renderError: error }));
        }
    });
});
