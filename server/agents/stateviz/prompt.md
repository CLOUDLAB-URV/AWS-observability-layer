...
- **Connection labels carry TWO segments in one string** — a **step number** and an **action** — separated by ` || ` (space, two pipes, space), in that order: `"<STEP> || <ACTION>"`. The app always shows the action, and shows or hides the step number with a user toggle; you always emit both. Draw EVERY arrow the SAME way — a light stroke with a legible light-grey label sitting on a dark pill (`style.fill: "#0d1117"`, which hides the line behind the text so labels never collide with the arrows or each other) — so the diagram stays clean and in harmony on the dark canvas:
  - `{ style.stroke: "#e6edf3"; style.stroke-width: 2; style.font-color: "#c9d1d9"; style.font-size: 15; style.fill: "#0d1117" }` with the segmented label, e.g. `"1 || GET /api/ec2"`, `"2 || Query orders"`, `"3 || Publish order.created"`.
  ...
  - **`<ACTION>`** — what TRAVELS down this edge, GROUNDED in the inventory (take the FIRST rung it supports, never a lower one, and never invent a route or name that isn't there):
    1. **The selector that picks this path**: an HTTP method + route (`"POST /orders"`), a CDN/listener path pattern (`"/api/*, /ws"`, `"/* (SPA)"`), a queue/topic/event name (`"Publish order.created"`, `"Consume orders-q"`), a mount point (`"Mount /app/persistence"`).
    2. **Verb + the named object** — object taken from the target's `name`, `purpose` or `details`: `"Query orders"`, `"Store invoices"`, `"Fetch secrets"`, `"Read/write state"`.
    3. **A role phrase**, only when nothing above is available: `"Forward to backend"`, `"Egress to internet"`. Never a bare verb (`"Invoke"`, `"Query"` alone): a verb with no object says nothing the arrow doesn't already.
  - **NEVER a protocol or port in the label** — not alone, not as prefix, suffix or bracket. `"HTTPS"`, `"HTTP port 8000"`, `"TCP Query"`, `"Invoke LLM HTTPS"`, `"Egress (HTTPS 443)"` are all WRONG: the resource panel already shows each connection's protocol and port on click, so the label's one job is the TOPOLOGY — what moves, and why down THIS edge.
  - **Sentence case** (`"Publish order.created"`, not `"Publish Order.Created"`), and one relationship worded the same everywhere — both copies of a replicated MULTI-AZ edge carry the identical label.
- **SIBLING EDGES MUST BE TOLD APART.** When one node has SEVERAL outgoing edges, each label says what routes traffic down THAT branch and no two may read alike: a CloudFront with two origins reads `"/* (SPA)"` to the bucket and `"/api/*, /ws"` to the load balancer; an ALB, one listener rule per branch. If the inventory truly can't distinguish them, name each destination's role (`"Forward to backend"` vs `"Serve static assets"`) — never the protocol, which makes every branch read alike.
...
- **TWO-HEADED ARROWS — when the point of the edge is what comes BACK.** Look at what the label names and which way that thing travels. If it travels toward whoever OPENED the connection, the edge gets a head at both ends: add a THIRD label segment `both`. If it travels toward the destination, leave it a plain single arrow. The declaration order never changes — it always runs from the initiator — and the marker never appears on the canvas.
  - ✅ two heads: `"11 || Pull image || both"` (the image arrives at the task), `"10 || Fetch secrets || both"`, `"7 || Query orders || both"`, `"6 || Read/write state || both"`, `"11 || Consume orders-q || both"`.
  - ✅ one head: `"5 || Forward to backend"`, `"9 || Send verification email"`, `"8 || Publish order.created"`, `"4 || Store invoices"`, `"3 || Egress to internet"` — the initiator hands something over and gets nothing back worth drawing.
  - On an HTTP route the METHOD decides: `GET` and `HEAD` fetch, so they take `both`; `POST`, `PUT`, `PATCH` and `DELETE` deliver, so they stay single.
  - Never put `both` on an edge that already carries `back`: an egress is one-way by definition.
- The sentinel ` || ` appears ONLY inside connection labels — never in a node or container label, and never inside either segment.
- **Budget for the action: 2 to 5 words, about 32 characters.** One segmented label per connection. The layout is computed with the labels REMOVED, so the text has to fit whatever gap the packed diagram leaves — but the app breaks any label past ~16 characters into two balanced lines by itself and keeps whichever form sits better, so ~30 characters is comfortable and only past ~50 does a label start crowding the icons. Spend that budget on MEANING, never on padding: `"Read/write orders"` beats `"Read/write"`, and both beat `"NFS"`. Cutting a label down to a protocol to save space is the one trade never worth making.
- Fixed wording for the pairs rung 3 would otherwise improvise, so they read the same in every diagram: → NAT/Internet Gateway `"Egress to internet"`; → secrets store `"Fetch secrets"`; → file system `"Read/write state"`; → mail `"Send verification email"`; → image registry `"Pull image"`; client → DNS `"Resolve <domain>"`; DNS → the target its record points at `"Alias to <target>"`; CDN/proxy → its origin, the path pattern it serves, which takes `both` because the object travels back (`"/* (SPA) to S3 || both"`); load balancer → compute `"Forward to <target>"`.
